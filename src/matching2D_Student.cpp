                      ...
                      std::vector<cv::DMatch> &matches, std::string descriptorType, std::string matcherType, std::string selectorType)
{
    ...
    //cout << "Matched keypoints " << matches.size() << " in " << 1000 * t / 1.0 << " ms" << endl;
}
...
void descKeypoints(vector<cv::KeyPoint> &keypoints, cv::Mat &img, cv::Mat &descriptors, string descriptorType)
{
    ...
    else if (descriptorType.compare("BRIEF") == 0)
    {
        ...
        cv::Ptr<cv::xfeatures2d::BriefDescriptorExtractor> extractor = cv::xfeatures2d::BriefDescriptorExtractor::create(bytes, use_orientation);
        extractor->compute(img, keypoints, descriptors);
    }
    ...
    //extractor->compute(img, keypoints, descriptors);
    t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
    //cout << descriptorType << descriptors.size() << " descriptor extraction in " << 1000 * t / 1.0 << " ms" << endl;
}
